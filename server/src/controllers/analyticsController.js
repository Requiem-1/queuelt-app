...
const getAnalyticsSummary = async (req, res) => {
  try {
    ...
    let avgWaitTimeMinutes = 12; // default estimate
    ...
    console.error('[analyticsController]: getAnalyticsSummary error:', error.message);
    res.status(500).json({
      success: false,
      message: 'Server error generating analytics summary',
      error: error.message,
    });
  }
};
